...
Consider $(H,\inne{\cdot}{\cdot}$, Hilbert space over $\comp$, \func{A}{{D_A\subset H}}{H} linear, with adjoint \func{\dual{A}}{D_\dual{A}\subset H}{H}. Recall $\dual{A}$ characterized by $\forall x\in D_A$, $y\in D_\dual{A}$: $\inne{\dual{A}y}{x}=\inne{y}{Ax}$ and $D_\dual{A}=\{y\in H : l_y:D_A\to \comp,\,x\mapsto \inne{y}{Ax}\,\text{ is continuous }\}$.
...
\begin{remark}[Subtleness  of "symmetric"]\nl
...
\begin{lemma}[Symmetric operator has real point spectrum]\nl
...
\begin{proposition}[Self-adjoint operator has real specturm]
...
\end{theorem}
\begin{example}\nl
	Shift operator \func{T_\lambda}{\ell^2}{\ell^2} continued. is compact $\iff \lim_{k\to\infty}\lambda_k=0$, self-adjoint $\iff \lambda_k\in\real\,\forall k$, and we know $\sigma_p(T_\lambda)=\overline{\{\lambda_k:k\in\natu\}}$.
\end{example}
