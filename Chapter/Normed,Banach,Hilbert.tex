...
\begin{theorem}[Fixed point]\rm \nextline
	...
	Then $f$ admits a unqiue fixed point $x^*\in X$, which can be found by
...
\begin{example}[Euclidean Space over $\mathbb C$]\rm\nextline
	...
	$$
	is a Hilbert space. This is a generalisation of last example. Still, this is a finite-dimensional Hilbert space.
\end{example}
...
Structure of inner product allows disscussion for nice geometric property of Hilbert spaces. This includes orthogonality, angles and nearest distance etc.

\subsubsection{Orthogonality}
Orthogonality is the generalisation of two lines being perpendicular. In euclidean geometry, we have Pythagorean theorem closely related to such property. Results on orthogonality in Hilbert spaces in many ways resemble their Euclidean version.
...
\begin{definition}[Orthogonal Complement]\rm\label{ortho comp}\nextline
	...
	that $f\perp g\,,\forall g \in A$. $A^\perp$ is always a subset of $\mathscr{H}$. Moreover, $A^\perp=\cup_{a\in A}(a)^\perp$, and we can prove thay $A^\perp$ is always a closed subset of \hbs.
\end{definition}
...
In Euclidean geometry, choosing a point and a line we can find the minimal distance form the point to any point on the line, defined as the distance from the point to the line. This can be generalised, with line extending to a {\bf{\emph{closed convex set}}} and space becoming a Hilbert space.
...
\begin{proposition}[Projection Theorem]\rm\nextline
	...
	\begin{enumerate}[(1)]
		...
		\item $Qx$ is the closest vettor in $M^\perp$ to x.
	\end{enumerate}
	\textit{proof:}\\
