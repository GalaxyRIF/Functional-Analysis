...
\begin{theorem}[Every metric space can be completed]\rm\nextline
	Result is put in completion of metric space in appendix.
\end{theorem}
...
\begin{proposition}[Completeness of product of Banach spaces]\label{Completeness of product of Banach spaces}\nl
Let $X$, $Y$ be Banach spaces, then $X\times Y=\{(x,y):x\in X,\,\,y\in Y\}$ endowed with norm $\norm{(x,y)}_{X\times Y}=\norm{x}_X+\norm{y}_Y$ is complete. Proof of this thoerem is left as an exercise.    
\end{proposition}
