...
\begin{example}\rm\nextline
	...
	$$
	First we consider the addition to be term-wise and multiplication apllied to whole sequence:
	$$
...
\begin{example}[Weak $L^p$ space]\rm\nextline
	Define $D_f(t)=\{\lambda{x\in\mathbb{R}:|f(x)|>t}\}$, where $\lambda$ is Lesbegue measure on $\mathbb{R}$. Consider:
	$$
		L^{p,w}=
		\left\{
		f:\mathbb{R}\xrightarrow{}\mathbb{R}:f,\,\,measureable,\,\, and\,\, \exists C>0,\,\,s.t.\,\,D_f(t)<\frac{C^p}{t_p}\,\forall t>0
		\right\}
	$$
...
\begin{definition}[dense]\rm
	A set in $S$ metric space $(V,d)$ is dense if there it intersects with any open subset of $V$. Equivilently this is $\forall x\in V,\forall \varepsilon>0$, $D\cap B_{x,\varepsilon}\neq \emptyset$.
\end{definition}
...
\begin{proposition}[Compact $\implies$ closed and bounded]\rm\nextline
	...
	\pf First we show that compact implies closed. Let $(x_n)$ be a convergent sequence in $K$, then by compactness, there is a subsequence of $(x_n)$, name it $(y_n)$ which converges to $y\in K$, but by uniqueness of limit $(x_n)$ also converges to $y\in K$.\\
...
\begin{theorem}[F.Riesz]\label{compact unit balls}\rm\nextline
	...
	\end{itemize}
	\pf Proof uses a lemma which is shown later\placeholder
\end{theorem}
